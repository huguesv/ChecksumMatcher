...
/// <summary>
/// Set of extension methods for using <see cref="IDispatcherQueueTimer"/>.
/// </summary>
internal static class DispatcherQueueTimerExtensions
{
    private static readonly ConcurrentDictionary<IDispatcherQueueTimer, Action> DebounceInstances = new();
    ...
    /// <example>
    /// <code>
    /// private IDispatcherQueueTimer _typeTimer = DispatcherQueueService.GetDispatcherQueue().CreateTimer();
    ///
    ...
    /// </code>
    /// </example>
    public static void Debounce(this IDispatcherQueueTimer timer, Action action, TimeSpan interval, bool immediate = false)
    {
        ...
        else
        {
            ...
            DebounceInstances.AddOrUpdate(timer, action, (k, v) => action);
        }
    ...
    private static void Timer_Tick(object? sender, object e)
    {
        ...
        if (sender is IDispatcherQueueTimer timer)
        {
            timer.Tick -= Timer_Tick;
            timer.Stop();

            if (DebounceInstances.TryRemove(timer, out var action))
            {
                action?.Invoke();
            }
        }

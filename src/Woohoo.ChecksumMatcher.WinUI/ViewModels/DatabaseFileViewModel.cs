...
public sealed partial class DatabaseFileViewModel : ObservableRecipient, IDisposable
{
        ...
        ILogger logger)
    {
        ...
        {
            var gameViewModel = new DatabaseGameViewModel(game, this.clipboardService, this.logger);
            this.Games.Add(gameViewModel);
            this.GamesByName[game.Name] = gameViewModel;
        }

        this.Metadata = new DatabaseMetadataViewModel(this.clipboardService, db);
        this.Stats = new DatabaseStatsViewModel(this.databaseService, this.clipboardService, this.logger, db);
    ...
    public List<DatabaseGameViewModel> Games { get; private set; } = [];

    public Dictionary<string, DatabaseGameViewModel> GamesByName { get; private set; } = [];
    ...
    private void CopyAllScanMatched()
    {
        try
        {
            this.clipboardService.SetText(string.Join(Environment.NewLine, this.ScanMatchedFiles.Select(item => item.ActualDisplayName).Order()));
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    [RelayCommand]
    private void CopyAllScanMatchedContainer()
    {
        try
        {
            var containers = this.ScanMatchedFiles.Select(f => f.ActualContainerName).Distinct().Order();
    ...
    [RelayCommand]
    private void CopyScanMatchedContainer(DatabaseScanResultActualViewModel viewModel)
    {
        try
        {
            this.clipboardService.SetText(viewModel.ActualContainerName);
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    private void CopyAllScanMissing()
    {
        try
        {
            this.clipboardService.SetText(string.Join(Environment.NewLine, this.ScanMissingFiles.Select(item => item.ExpectedDisplayName).Order()));
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    [RelayCommand]
    private void CopyAllScanMissingContainer()
    {
        try
        {
            var containers = this.ScanMissingFiles.Select(f => f.ExpectedContainerName).Distinct().Order();
            this.clipboardService.SetText(string.Join(Environment.NewLine, containers));
        }
        catch (Exception ex)
        {
    ...
    [RelayCommand]
    private void CopyScanMissingContainer(DatabaseScanResultExpectedViewModel viewModel)
    {
        try
        {
    ...
    private void CopyAllScanUnused()
    {
        try
        {
            this.clipboardService.SetText(string.Join(Environment.NewLine, this.ScanUnusedFiles.Select(item => item.ActualDisplayName).Order()));
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    [RelayCommand]
    private void CopyAllScanUnusedContainer()
    {
        try
        {
            var containers = this.ScanUnusedFiles.Select(f => f.ActualContainerName).Distinct().Order();
            this.clipboardService.SetText(string.Join(Environment.NewLine, containers));
        }
        catch (Exception ex)
        {
    ...
    [RelayCommand]
    private void CopyScanUnusedContainer(DatabaseScanResultActualViewModel viewModel)
    {
        try
        {
            this.clipboardService.SetText(viewModel.ActualContainerName);
    ...
    private void CopyAllScanWrongNamed()
    {
        try
        {
            this.clipboardService.SetText(string.Join(Environment.NewLine + Environment.NewLine, this.ScanIncorrectNameFiles.Select(item => $"Actual:   {item.ActualDisplayName}{Environment.NewLine}Expected: {item.ExpectedDisplayName}").Order()));
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    private void CopyAllScanWrongNamedActual()
    {
        try
        {
            this.clipboardService.SetText(string.Join(Environment.NewLine, this.ScanIncorrectNameFiles.Select(item => item.ActualDisplayName).Order()));
        }
        catch (Exception ex)
        {
            this.logger.LogError(ex, "Error processing command.");
    ...
    [RelayCommand]
    private void CopyAllScanWrongNamedActualContainer()
    {
        try
        {
            var containers = this.ScanIncorrectNameFiles.Select(f => f.ActualContainerName).Distinct().Order();
            this.clipboardService.SetText(string.Join(Environment.NewLine, containers));
        }
        catch (Exception ex)
        {
    ...
    [RelayCommand]
    private void CopyAllScanWrongNamedExpectedContainer()
    {
        try
        {
            var containers = this.ScanIncorrectNameFiles.Select(f => f.ExpectedContainerName).Distinct().Order();
            this.clipboardService.SetText(string.Join(Environment.NewLine, containers));
        }
        catch (Exception ex)
        {
    ...
    [RelayCommand]
    private void CopyScanWrongNamedActualContainer(DatabaseScanResultWrongNameViewModel viewModel)
    {
        try
        {
            this.clipboardService.SetText(viewModel.ActualContainerName);
    ...
    [RelayCommand]
    private void CopyScanWrongNamedExpectedContainer(DatabaseScanResultWrongNameViewModel viewModel)
    {
        this.clipboardService.SetText(viewModel.ExpectedContainerName);
    }
    ...
    private void DatabaseService_ScanProgress(object? sender, ScanEventArgs e)
    {
        ...
        this.dispatcherQueue.TryEnqueue(() =>
        {
            ...
            {
                this.logger.LogError(ex, "Error processing scan progress event.");
            }
        });
    }
    ...
    private void AddScanResults(DatabaseScanResults results)
    {
        ...
        foreach (var wrongNamed in results.WrongNamed)
        {
            ...
            this.GamesByName.TryGetValue(wrongNamed.Rom.ContainerName, out var game);
            if (game is not null)
            {
                var rom = game.Roms.FirstOrDefault(temp => temp.Name == wrongNamed.Rom.RomRelativeFilePath);
                if (rom is not null)
                {
        ...
        foreach (var matched in results.Matched)
        {
            ...
            this.GamesByName.TryGetValue(matched.Rom.ContainerName, out var game);
            if (game is not null)
            {
                var rom = game.Roms.FirstOrDefault(temp => temp.Name == matched.Rom.RomRelativeFilePath);
                rom?.Status = DatabaseRomStatus.Found;
    ...
    private void DatabaseService_RebuildProgress(object? sender, RebuildEventArgs e)
    {
        ...
        this.dispatcherQueue.TryEnqueue(() =>
        {
            ...
            {
                this.logger.LogError(ex, "Error processing rebuild progress event.");
            }
        });
    }
    ...
    {
        if (!this.isLoadingRebuildSettings && value is not null)
        {
            this.SaveRebuildSettingsAsync(CancellationToken.None).FireAndForget((ex) => this.logger.LogError(ex, "Error saving rebuild settings."));
        }
    }

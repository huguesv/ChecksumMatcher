...
public sealed class OfflineExplorerService : IOfflineExplorerService
{
    ...
    public async Task<OfflineDiskCreateResults> CreateDiskAsync(string sourceFolderPath, string targetDiskFilePath, string diskName, OfflineDiskCreateSettings settings, CancellationToken ct)
    {
        ...
        try
        {
            return await Task.Run(() => CreateDisk(sourceFolderPath, targetDiskFilePath, diskName, settings, ct), ct);
        }
        catch (OperationCanceledException)
        {
            this.DiskCreateProgress?.Invoke(this, new OfflineDiskCreateProgressEventArgs(operationId, OfflineDiskCreateStatus.Canceled, diskName, 0, 0, 0, null));
            throw;
        }

        OfflineDiskCreateResults CreateDisk(string sourceFolderPath, string targetDiskFilePath, string diskName, OfflineDiskCreateSettings settings, CancellationToken ct)
        {
            this.DiskCreateProgress?.Invoke(this, new OfflineDiskCreateProgressEventArgs(operationId, OfflineDiskCreateStatus.Pending, diskName, 0, 0, 0, null));
            ...
            IndexerProgressEventArgs? lastScanEvent = null;
            ...
            var result = new OfflineDiskCreateResults
            {
                FolderCount = lastScanEvent?.FolderCount ?? 0,
                FileCount = lastScanEvent?.FileCount ?? 0,
                ArchiveItemCount = lastScanEvent?.ArchiveItemCount ?? 0,
                TimeSpan = lastScanEvent?.TimeSpan,
            };

            this.DiskCreateProgress?.Invoke(this, new OfflineDiskCreateProgressEventArgs(
                operationId,
                OfflineDiskCreateStatus.Completed,
                diskName,
                result.FolderCount,
                result.FileCount,
            ...
            void OnProgressChanged(object? sender, IndexerProgressEventArgs e)
            {
                ...
                this.DiskCreateProgress?.Invoke(this, new OfflineDiskCreateProgressEventArgs(
                    operationId,
                    OfflineDiskCreateStatus.Scanning,
                    diskName,
                    e.FolderCount,
                    e.FileCount,
                    e.ArchiveItemCount,

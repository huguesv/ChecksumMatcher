...
public class DatabaseService : IDatabaseService
{
    ...
    public DatabaseService(ILocalSettingsService localSettingsService, IOfflineExplorerService offlineExplorerService)
    {
    ...
    public async Task RebuildAsync(DatabaseFile file, RebuildSettings rebuildSettings, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(file);
    ...
    private async Task<EffectiveScanSettings> GetEffectiveScanSettingsAsync(DatabaseFile databaseFile, RomDatabase db, CancellationToken ct)
    {
        ...
        while (parentFolder is not null)
        {
            ...
            if (folderScanSettings.UseOnlineFolders)
            {
                ...
                {
                    var onlineFolderPath = Path.Combine(onlineFolder.FolderPath, db.Name);
            ...
            if (folderScanSettings.UseOfflineFolders)
            {
                ...
                {
                    var offlineFolderPath = Path.Combine(offlineFolder.FolderPath, db.Name);

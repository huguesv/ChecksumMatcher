...
public sealed class DatabaseService : IDatabaseService
{
    ...
    public async Task ScanAsync(DatabaseFile file, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (this.databaseCache.TryGetValue(file.FullPath, out var cachedDatabase))
            {
                this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = cachedDatabase, ProgressPercentage = 0, Status = ScanStatus.Pending, Results = new DatabaseScanResults() });
            }
            else
            {
                this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = ScanStatus.Pending, Results = new DatabaseScanResults() });
            }
        ...
        {
            this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 100, Status = ScanStatus.Canceled, Results = new DatabaseScanResults() });
            throw;
        }
        ...
        async Task DoScanAsync()
        {
            ...
            this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = db, ProgressPercentage = 0, Status = ScanStatus.Started, Results = new DatabaseScanResults() });
            ...
            this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = db, ProgressPercentage = 0, Status = ScanStatus.Scanning, Results = new DatabaseScanResults() });

            this.scanResultsCache.TryRemove(db.Name, out _);

            this.scanResultsCache[db.Name] = await Scanner.ScanAsync((ea) => this.ScanProgress?.Invoke(this, ea), operationId, file, db, scanSettings, this.offlineExplorerService.FindDiskByNameAsync, ct);
        }
    }
    ...
    public async Task ClearScanResultsAsync(DatabaseFile file, CancellationToken ct)
    {
        ...
        this.ScanProgress?.Invoke(this, new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = db, ProgressPercentage = 0, Status = ScanStatus.Cleared, Results = new DatabaseScanResults() });
    }
    ...
    public async Task RebuildAsync(DatabaseFile file, CancellationToken ct)
    {
        ...
        try
        {
            ...
            this.RebuildProgress?.Invoke(this, new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = RebuildStatus.Pending, Results = new DatabaseRebuildResults() });

            await Task.Run(DoRebuildAsync, ct);
        }
        catch (OperationCanceledException)
        {
            this.RebuildProgress?.Invoke(this, new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 100, Status = RebuildStatus.Canceled, Results = new DatabaseRebuildResults() });
            throw;
        }
    ...
    public async Task CreateDatabaseAsync(string sourceFolderPath, string targetDatabaseFilePath, DatabaseCreateSettings settings, CancellationToken ct)
    {
        ...
        try
        {
            ...
            this.DatabaseCreateProgress?.Invoke(
                this,
                new DatabaseCreateEventArgs
                {
                    OperationId = operationId,
                    DatabaseName = settings.Name,
                    ProgressPercentage = 0,
                    Status = DatabaseCreateStatus.Pending,
        ...
        catch (OperationCanceledException)
        {
            this.DatabaseCreateProgress?.Invoke(
                ...
                new DatabaseCreateEventArgs
                {
                    ...
                    DatabaseName = settings.Name,
                    ProgressPercentage = 0,
                    Status = DatabaseCreateStatus.Canceled,
                    Results = new DatabaseCreateResults(),
                });
            throw;
        }
    ...
    private static CloneMode DetectCloneModeFromDatabaseFilePath(DatabaseFile file)
    {

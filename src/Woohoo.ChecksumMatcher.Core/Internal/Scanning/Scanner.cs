...
internal static class Scanner
{
        ...
        CancellationToken ct)
    {
        ...
        var usedWithPerfectName = new HashSet<FileInformation>();
        ...
        var filesBySize = new Dictionary<long, List<FileInformation>>();
        foreach (var f in files)
        {
            if (!filesBySize.TryGetValue(f.Size, out var list))
            {
                list = [];
                filesBySize[f.Size] = list;
            }
        ...
        foreach (var disk in db.GetAllDisks())
        {
            ...
            var diskFileName = GetDiskFileNameWithExtension(disk);

            var checksumMatches = files.Where(file => Scanner.DoesChecksumMatch(disk, file)).ToArray();
            if (checksumMatches.Length > 0)
            {
                foreach (var checksumMatch in checksumMatches)
                {
                    ...
                    if (checksumMatch.FileRelativePath == diskFileName)
                    {
                        usedWithPerfectName.Add(checksumMatch);

                        var result = new RomAndFileMoniker(
                            new RomMoniker(disk.ParentGame.Name, diskFileName),
        ...
        foreach (var rom in db.GetAllRoms())
        {
            ...
            // Find all files that match the rom size using the dictionary
            filesBySize.TryGetValue(rom.Size, out var sizeMatchesList);
            var sizeMatches = sizeMatchesList ?? [];
                ...
                if (checksumMatches.Length > 0)
                {
                    ...
                    if (gameMatch != null)
                    {
                        if (gameMatch.FileRelativePath == rom.Name)
                        {
                            ...
                            _ = sizeMatches.Remove(gameMatch);
                            usedWithPerfectName.Add(gameMatch);
        ...
        // Send any pending progress
        ThrottleScanProgress(forceSend: true);

        foreach (var unusedFile in files.Except(usedWithPerfectName).Except(usedButIncorrectName))
        {
        ...
        reportProgress(new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = db, ProgressPercentage = 0, Status = ScanStatus.Scanning, Results = new DatabaseScanResults() { Unused = [.. unused] } });

        reportProgress(new ScanEventArgs { OperationId = operationId, DatabaseFile = file, Database = db, ProgressPercentage = 100, Status = ScanStatus.Completed, Results = new DatabaseScanResults() });
        ...
        void ThrottleScanProgress(RomAndFileMoniker? matched = null, RomAndFileMoniker? wrongNamed = null, RomMoniker? missing = null, bool forceSend = false)
        {
            ...
            {
                pendingProgressMatched.Add(matched);
            }

            if (wrongNamed is not null)
            {
                pendingProgressWrongNamed.Add(wrongNamed);
            }
            ...
            if (!forceSend && (DateTime.Now - lastProgress).TotalSeconds < 1)
            {
                return;
            }
        ...
        CancellationToken ct)
    {
        ...
        reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = RebuildStatus.Started, Results = new DatabaseRebuildResults() });
        ...
        foreach (var sourceFile in sourceFiles)
        {
            ...
            await EnsureChecksumsAsync(reportProgress, operationId, file, sourceFile, rebuildSettings, ct);

            if (sourceFile.IsDiskFile)
            {
                var matchedDisks = disks.Where(disk => Scanner.DoesChecksumMatch(disk, sourceFile)).ToArray();
                ...
                else
                {
                    ...
                    reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = RebuildStatus.Scanning, Results = new DatabaseRebuildResults() { Unused = [result] } });
                }
            }
            else
            {
        ...
        {
            await CheckForMissingCueFilesAsync(reportProgress, operationId, file, db, rebuildSettings, cueFolders, ct);
        }
        ...
        static async Task RebuildDiskFileAsync(Action<RebuildEventArgs> reportProgress, string operationId, DatabaseFile databaseFile, FileInformation sourceFile, RomDisk[] matchedDisks, string targetFolderPath, bool removeSource, CancellationToken ct)
        {
            ...
            {
                foreach (var matchedDisk in matchedDisks)
                {
                    ct.ThrowIfCancellationRequested();

                    if (await sourceContainer.ExistsAsync(sourceFile, ct))
                    {
                        var copier = FileCopierExtensionProvider.GetCopier(sourceFile, KnownContainerTypes.Folder, []);
                        if (copier != null)
                        {
                            var diskMoniker = new RomMoniker(matchedDisk.ParentGame.Name, GetDiskFileNameWithExtension(matchedDisk));

                            reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = databaseFile, ProgressPercentage = 0, Status = RebuildStatus.Building, Results = new DatabaseRebuildResults(), BuildingRom = diskMoniker });

                            await copier.CopyAsync(sourceFile, targetFolderPath, removeSource, matchedDisks.Length == 1, matchedDisk.ParentGame.Name, GetDiskFileNameWithExtension(matchedDisk), [], ct);

                            var result = new RomAndFileMoniker(diskMoniker, fileMoniker);
                            reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = databaseFile, ProgressPercentage = 0, Status = RebuildStatus.Scanning, Results = new DatabaseRebuildResults() { Matched = [result] } });
                        }
                    }
                }
                ...
                foreach (var matchedRom in matchedRoms)
                {
                    ...
                    if (await sourceContainer.ExistsAsync(sourceFile, ct))
                    {
                        ...
                        if (copier != null)
                        {
                            ...
                            reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = databaseFile, ProgressPercentage = 0, Status = RebuildStatus.Building, Results = new DatabaseRebuildResults(), BuildingRom = romMoniker });
        ...
        static async Task CheckForMissingCueFilesAsync(Action<RebuildEventArgs> reportProgress, string operationId, DatabaseFile file, RomDatabase db, RebuildSettings rebuildSettings, string[] cueFolders, CancellationToken ct)
        {
            FileInformation[]? rebuiltFiles = null;
            ...
            foreach (var game in db.Games)
            {
                ...
                foreach (var cueRom in missingCueRoms)
                {
                    ...
                    await EnsureChecksumsAsync(reportProgress, operationId, file, cueSourceFile, rebuildSettings, ct);
        ...
        static async Task EnsureChecksumsAsync(Action<RebuildEventArgs> reportProgress, string operationId, DatabaseFile file, FileInformation sourceFile, RebuildSettings rebuildSettings, CancellationToken ct)
        {
            if (!sourceFile.AllChecksumsCalculated)
            {
                ...
                if (container != null)
                {
                    ...
                    {
                        reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = RebuildStatus.Hashing, Results = new DatabaseRebuildResults(), HashingFile = new FileMoniker(sourceFile.ContainerAbsolutePath, sourceFile.ContainerName, sourceFile.FileRelativePath, sourceFile.IsFromOfflineStorage) });

                        await container.CalculateChecksumsAsync(sourceFile, ct);

                        reportProgress(new RebuildEventArgs { OperationId = operationId, DatabaseFile = file, ProgressPercentage = 0, Status = RebuildStatus.Scanning, Results = new DatabaseRebuildResults() });
                    }
                }
        ...
        CancellationToken ct)
    {
        ...
        reportProgress(new DatabaseCreateEventArgs
        {
            OperationId = operationId,
            DatabaseName = db.Name,
            ProgressPercentage = 0,
            Status = DatabaseCreateStatus.Started,
            Results = new DatabaseCreateResults(),
        });
        ...
        foreach (var folderInfo in new DirectoryInfo(sourceFolderPath).EnumerateDirectories())
        {
            ...
            foreach (var fileInformation in children)
            {
                ...
                if (fileInformation.IsDiskFile)
                {
                    AddDiskToGameAsync(reportProgress, operationId, fileInformation, db, game, settings.ForceCalculateChecksums, ct);
                }
                else
                {
                    await AddRomToGameAsync(reportProgress, operationId, fileInformation, db, game, settings.ForceCalculateChecksums, ct);
                }
            }
        }
        ...
        reportProgress(new DatabaseCreateEventArgs
        {
            ...
            Status = DatabaseCreateStatus.Completed,
            Results = new DatabaseCreateResults(),
        });

        static void AddDiskToGameAsync(Action<DatabaseCreateEventArgs> reportProgress, string operationId, FileInformation sourceFile, RomDatabase db, RomGame game, bool forceCalculateChecksums, CancellationToken ct)
        {
            var disk = new RomDisk(game)
            {
                Name = DiskPathWithoutExtension(sourceFile.FileRelativePath),
                SHA1 = sourceFile.ReportedDiskSHA1,
            };

            game.Disks.Add(disk);

            var fileMoniker = new FileMoniker(sourceFile.ContainerAbsolutePath, sourceFile.ContainerName, sourceFile.FileRelativePath, sourceFile.IsFromOfflineStorage);
            var romMoniker = new RomMoniker(game.Name, disk.Name);
            var result = new RomAndFileMoniker(romMoniker, fileMoniker);
            ...
            reportProgress(new DatabaseCreateEventArgs
            {
                ...
                DatabaseName = db.Name,
                ProgressPercentage = 0,
                Status = DatabaseCreateStatus.Scanning,
                Results = new DatabaseCreateResults() { Added = [result] },
            });
        }

        static string DiskPathWithoutExtension(string path)
        {
            return Path.Combine(Path.GetDirectoryName(path) ?? string.Empty, Path.GetFileNameWithoutExtension(path)) ?? string.Empty;
        }

        static async Task AddRomToGameAsync(Action<DatabaseCreateEventArgs> reportProgress, string operationId, FileInformation sourceFile, RomDatabase db, RomGame game, bool forceCalculateChecksums, CancellationToken ct)
        {
            await EnsureChecksumsAsync(reportProgress, operationId, sourceFile, db, forceCalculateChecksums, ct);
        ...
        static async Task EnsureChecksumsAsync(Action<DatabaseCreateEventArgs> reportProgress, string operationId, FileInformation sourceFile, RomDatabase db, bool forceCalculateChecksums, CancellationToken ct)
        {
            if (!sourceFile.AllChecksumsCalculated)
            {
                var container = ContainerExtensionProvider.GetContainer(sourceFile.ContainerAbsolutePath);
                if (container != null)
                {
                    if (forceCalculateChecksums || sourceFile.ReportedCRC32.Length == 0)
                    {
                        reportProgress(new DatabaseCreateEventArgs
                        {
                            ...
                            DatabaseName = db.Name,
                            ProgressPercentage = 0,
                            Status = DatabaseCreateStatus.Hashing,
                            Results = new DatabaseCreateResults(),
                            HashingFile = new FileMoniker(sourceFile.ContainerAbsolutePath, sourceFile.ContainerName, sourceFile.FileRelativePath, sourceFile.IsFromOfflineStorage),
                        });
    ...
    private static string GetDiskFileNameWithExtension(RomDisk disk)
    {
        ArgumentNullException.ThrowIfNull(disk);

        return disk.Name + ".chd";
    }

    private static bool DoesChecksumMatch(RomDisk disk, FileInformation file)
    {
        ArgumentNullException.ThrowIfNull(disk);
        ArgumentNullException.ThrowIfNull(file);
        ...
        if (file.IsDiskFile)
        {
            if (disk.SHA1.Length > 0 && file.ReportedDiskSHA1.Length > 0)
            {
                return disk.SHA1.SequenceEqual(file.ReportedDiskSHA1);
            }
        }

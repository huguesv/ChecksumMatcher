        ...
        CancellationToken ct)
    {
        ...
        foreach (var sourceFile in sourceFiles)
        {
            ...
            if (matchedRoms.Length > 0)
            {
                ...
                await RebuildFileAsync(reportProgress, operationId, file, sourceFile, matchedRoms, rebuildSettings.TargetFolderPath, rebuildSettings.TargetContainerType, rebuildSettings.RemoveSource, ct);
        ...
        static async Task RebuildFileAsync(Action<RebuildEventArgs> reportProgress, string operationId, DatabaseFile databaseFile, FileInformation sourceFile, RomFile[] matchedRoms, string targetFolderPath, string targetContainerType, bool removeSource, CancellationToken ct)
        {
        ...
        static async Task CheckForMissingCueFilesAsync(Action<RebuildEventArgs> reportProgress, string operationId, DatabaseFile file, RomDatabase db, RebuildSettings rebuildSettings, string[] cueFolders, CancellationToken ct)
        {
            ...
            foreach (var game in db.Games)
            {
                ...
                foreach (var cueRom in missingCueRoms)
                {
                    ...
                    Directory.CreateDirectory(rebuildSettings.TargetFolderPath);
                    await RebuildFileAsync(reportProgress, operationId, file, cueSourceFile, [cueRom], rebuildSettings.TargetFolderPath, rebuildSettings.TargetContainerType, removeSource: false, ct: ct);
                }
            }
        }

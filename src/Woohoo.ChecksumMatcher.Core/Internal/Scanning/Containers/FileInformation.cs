...
internal sealed class FileInformation
{
    ...
    public bool IsDiskFile => string.Equals(Path.GetExtension(this.FileRelativePath), ".chd", StringComparison.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Gets or sets the SHA1 that is reported by the disk (chd) header.
    /// </summary>
    public byte[] ReportedDiskSHA1 { get; set; }

    /// <summary>
    /// Gets or sets the disk version that is reported by the disk (chd) header.
    /// </summary>
    public int ReportedDiskVersion { get; set; }

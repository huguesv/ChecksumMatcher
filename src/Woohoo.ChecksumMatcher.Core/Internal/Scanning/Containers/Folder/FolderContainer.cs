        ...
        foreach (var fileInfo in folderInfo.GetFiles("*.*", searchOption))
        {
            ...
            if (compressedContainer == null)
            {
                ...
                if (file.IsDiskFile)
                {
                    try
                    {
                        var chd = new ChdFile(file.ContainerAbsolutePath + Path.DirectorySeparatorChar + file.FileRelativePath);
                        file.ReportedDiskSHA1 = chd.SHA1;
                        file.ReportedDiskVersion = chd.FormatVersion;
                    }
                    catch
                    {
                    }

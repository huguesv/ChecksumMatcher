...
public sealed class ClrMameImporter : IDatabaseImporter
{
    ...
    public RomDatabase Import(string text)
    {
        ...
        while (lineIndex < lineCount)
        {
            if (lines[lineIndex].StartsWith("clrmamepro", StringComparison.OrdinalIgnoreCase))
            {
                lineIndex = ReadHeader(db, lines, lineCount, lineIndex);
            }
    ...
    private static bool GetNextPair(string text, ref int index, out string name, out string val)
    {
        ...
        var endNameIndex = text.IndexOf(' ', index + 1);
        if (endNameIndex > index)
        {
            ...
            {
                endValIndex = text.IndexOf('"', endNameIndex + 2);
            }
            else
            {
                endValIndex = text.IndexOf(' ', endNameIndex + 2);
    ...
    private static int ReadHeader(RomDatabase db, string[] lines, int lineCount, int lineIndex)
    {
        lineIndex++;

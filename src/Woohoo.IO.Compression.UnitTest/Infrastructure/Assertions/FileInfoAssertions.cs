...
namespace Woohoo.IO.Compression.UnitTest.Infrastructure.Assertions;
...
internal class FileInfoAssertions : ReferenceTypeAssertions<FileInfo, FileInfoAssertions>
{
    ...
    [CustomAssertion]
    public AndConstraint<FileInfoAssertions> BeTorrentZip(string because = "", params object[] becauseArgs)
    {
        this.chain
            .BecauseOf(because, becauseArgs)
            .Given(() => this.Subject.FullName)
            .ForCondition(IsTorrentZip)
            .FailWith("Expected {context:file} to be torrentzip{reason}.");

        return new AndConstraint<FileInfoAssertions>(this);
    }

    [CustomAssertion]
    public AndConstraint<FileInfoAssertions> BeTorrentSevenZip(string because = "", params object[] becauseArgs)
    {
        this.chain
            .BecauseOf(because, becauseArgs)
            .Given(() => this.Subject.FullName)
            .ForCondition(IsTorrentSevenZip)
            .FailWith("Expected {context:file} to be torrent7z{reason}.");

...
public sealed class Indexer
{
    ...
    private OfflineItem CreateFolder(DirectoryInfo folderInfo, OfflineItem? parentItem, OfflineDisk parentDisk)
    {
        ...
        return new OfflineItem
        {
            ...
            Created = folderInfo.CreationTime,
            Modified = folderInfo.LastWriteTime,
            IsReadOnly = folderInfo.Attributes.HasFlag(FileAttributes.ReadOnly),
            IsSystem = folderInfo.Attributes.HasFlag(FileAttributes.System),
            IsHidden = folderInfo.Attributes.HasFlag(FileAttributes.Hidden),
    ...
    private OfflineItem CreateFile(FileInfo fileInfo, OfflineItem? parentItem, OfflineDisk parentDisk)
    {
        ...
        else
        {
            ...
            if (string.Equals(Path.GetExtension(fileInfo.FullName), ".chd", StringComparison.OrdinalIgnoreCase))
            {
                var chd = new ChdFile(fileInfo.FullName);
                try
                {
                    fileItem.ReportedDiskSHA1 = HashCalculator.HexToString(chd.SHA1);
                    fileItem.ReportedDiskVersion = chd.FormatVersion;
                }
                catch
                {
                }

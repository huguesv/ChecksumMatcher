...
use --cmpro parameter to use with cmpro.
after cmpro finishes update, execute t7z . over roms directory to convert modified non-solid archives to solid t7zipped.
alternatively, use --batch option(or no options at all) instead of --cmpro to avoid after-step,
but keep in mind that it will be slow as hell.
